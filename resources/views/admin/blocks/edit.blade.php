...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 class="font-headline-md text-headline-md font-bold text-primary dark:text-primary-fixed mb-2">Edit Blok: {{ strtoupper($block->nama_blok) }}</h1>
        <p class="font-body-md text-body-md text-on-surface-variant">Perbarui informasi data blok properti.</p>
    </div>
...
<div class="bg-surface-container dark:bg-surface-container-low rounded-3xl p-6 md:p-8 border border-outline-variant dark:border-outline">
    <form action="{{ route('admin.blocks.update', $block->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="nama_blok" value="{{ old('nama_blok', $block->nama_blok) }}" required
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="4"
                      class="w-full bg-surface-bright dark:bg-surface-dim border border-outline-variant dark:border-outline rounded-xl px-4 py-3 text-on-surface focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all"
                      placeholder="Masukkan deskripsi blok (opsional)...">{{ old('deskripsi', $block->deskripsi) }}</textarea>
        </div>

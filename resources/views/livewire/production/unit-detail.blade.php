<div>
    ...
    .border-l-4.border-primary { border-left-color: #10B981 !important; }
    .rounded-full.bg-secondary { background-color: #10B981 !important; }
    .material-symbols-outlined.text-primary { color: #10B981 !important; }
    .font-headline-md.text-2xl.font-bold.text-primary { color: #10B981 !important; }
    .font-label-sm.text-xs.text-primary.font-bold { color: #10B981 !important; }
    </style>

    <div class="mb-6">
        <a href="{{ route('production.dashboard') }}" wire:navigate class="font-label-md text-sm text-primary hover:text-primary-fixed-dim flex items-center gap-1 transition-colors">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm" wire:ignore>
            ...
            <div class="bg-surface-container-low rounded-xl border border-outline-variant p-6 shadow-sm">
                ...
                <div class="space-y-4">
                    ...
                    @foreach($sortedHistory as $progress)
                        <div class="border-l-4 border-primary pl-4 py-2 hover:bg-surface-container-high transition-colors rounded-r-lg" wire:key="prog-{{ $progress->id }}">
        ...
        <div class="space-y-6">
            <div class="bg-surface-container-low rounded-xl border border-outline-variant p-5 shadow-sm">
                ...
                <div class="space-y-3">
                    <button wire:click="openModal" class="block w-full py-2.5 bg-primary text-on-primary rounded-lg font-label-md text-sm text-center hover:bg-primary-dark transition-colors shadow-sm flex justify-center items-center gap-2">
            ...
            <div class="bg-surface-container-low rounded-xl border border-outline-variant p-5 shadow-sm">
                ...
                <div class="space-y-4">
                    @foreach($progressHistory->take(5) as $activity)
                        <div class="flex gap-3" wire:key="timeline-{{ $activity->id }}">
        ...
        <div class="bg-surface rounded-2xl border border-outline-variant shadow-lg w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]" @click.outside="$wire.closeModal()">
            ...
            <div class="p-6 overflow-y-auto">
                <form wire:submit="saveProgress" class="space-y-4">
                    <div>
                        <label class="block font-label-md text-on-surface mb-1">Tahap <span class="text-error">*</span></label>
                        <select wire:model="tahap" class="w-full rounded-lg border-outline-variant bg-surface-container-lowest text-on-surface px-4 py-2 focus:ring-primary focus:border-primary transition-colors" required>
                        ...
                        @error('tahap') <span class="text-error text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block font-label-md text-on-surface mb-1">Persentase (%) <span class="text-error">*</span></label>
                        <input type="number" wire:model="persentase" min="0" max="100" class="w-full rounded-lg border-outline-variant bg-surface-container-lowest text-on-surface px-4 py-2 focus:ring-primary focus:border-primary transition-colors" required>
                        @error('persentase') <span class="text-error text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block font-label-md text-on-surface mb-1">Catatan</label>
                        <textarea wire:model="catatan" rows="3" class="w-full rounded-lg border-outline-variant bg-surface-container-lowest text-on-surface px-4 py-2 focus:ring-primary focus:border-primary transition-colors" placeholder="Opsional..."></textarea>
                        @error('catatan') <span class="text-error text-xs mt-1 block">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block font-label-md text-on-surface mb-1">Upload Foto</label>
                        <input type="file" wire:model="foto" accept="image/*" class="w-full text-sm text-on-surface-variant file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary-container file:text-on-primary-container hover:file:bg-primary-container/80 transition-colors">
                        <div wire:loading wire:target="foto" class="text-xs text-primary mt-1">Uploading...</div>
                        @error('foto') <span class="text-error text-xs mt-1 block">{{ $message }}</span> @enderror
                        @if ($foto)
                            <div class="mt-2">
                                <img src="{{ $foto->temporaryUrl() }}" class="w-24 h-24 object-cover rounded-lg border border-outline-variant">
                            </div>
                        @endif
                    </div>
